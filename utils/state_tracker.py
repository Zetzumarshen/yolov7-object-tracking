...
class BBoxState:
    def __init__(self, object_id, bbox_xyxy, class_label, class_confidence, orientation_label = None, timestamp=None):
        ...
        if not timestamp: # ipcamera streaming
            self.timestamp = d.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        else: 
...
class StateTracker:
    def __init__(self, line_roi, fps, in_orientation=None, is_webcam = False, master=None):
        """
        ...
        self.motorcycle_in_count = 0
        self.motorcycle_out_count = 0
        self.bus_in_count = 0
    ...
    def update_state_tracker_in_out_counter(self):
        ...
        car_in_count = 0
        car_out_count = 0
        motorcycle_in_count = 0
        motorcycle_out_count = 0
        bus_in_count = 0
        bus_out_count = 0

        for d in ds:
            obj = self.get_highest_confidence_by_id(d['object_id'])
            d['class_label'] = obj['class_label']
            if d['count'] == "in":
                if d['class_label'] == 'truck':
                    truck_in_count += 1
                elif d['class_label'] == 'car':
                    car_in_count += 1
                elif d['class_label'] == 'motorcycle':
                    motorcycle_in_count += 1
                elif d['class_label'] == 'bus':
                    bus_in_count += 1    
        ...
        if self.is_counts_changed(truck_in_count, truck_out_count, car_in_count, car_out_count,
                       motorcycle_in_count, motorcycle_out_count, bus_in_count, bus_out_count) and self.master is not None:
            self.master.counts_changed()

        self.truck_in_count = truck_in_count
        self.truck_out_count = truck_out_count
        self.car_in_count = car_in_count
        self.car_out_count = car_out_count
        self.motorcycle_in_count = motorcycle_in_count
        self.motorcycle_out_count = motorcycle_out_count
        self.bus_in_count = bus_in_count
        self.bus_out_count = bus_out_count

    def is_counts_changed(self, truck_in_count, truck_out_count, car_in_count, car_out_count,
                       motorcycle_in_count, motorcycle_out_count, bus_in_count, bus_out_count):
        # Check if any of the counts have changed
        if (truck_in_count != self.truck_in_count or
            truck_out_count != self.truck_out_count or
            car_in_count != self.car_in_count or
            car_out_count != self.car_out_count or
            motorcycle_in_count != self.motorcycle_in_count or
            motorcycle_out_count != self.motorcycle_out_count or
            bus_in_count != self.bus_in_count or
            bus_out_count != self.bus_out_count):
            return True
        else:
            return False
    ...
    def get_highest_confidence_by_id(self, object_id):
        labels = self.get_highest_confidence()
        for label in labels:
            if object_id == label['object_id']:
                return label
    ...
    def detect_change_in_out_counter(self):
        ...
        for frame_number, frame in enumerate(self.state_history):
            for bbox in frame:
                obj_id = bbox['object_id']
                obj_label = self.get_highest_confidence_by_id(obj_id)
                count = bbox['in_out_counter']
                if frame_number == 0:
                    if count == 1:
                        result.append({'object_id': obj_id, 'class_label' : obj_label['class_label'], 'class_confidence': obj_label['class_confidence'], 'frame_number': frame_number, 'count': 'in',  'timestamp': bbox['timestamp']})
                    elif count == -1:
                        result.append({'object_id': obj_id, 'class_label' : obj_label['class_label'], 'class_confidence': obj_label['class_confidence'], 'frame_number': frame_number, 'count': 'out',  'timestamp': bbox['timestamp']})
                else:
                    ...
                    if prev_count == 0 and count == 1:
                        ...
                        result.append({'object_id': obj_id, 'class_label' : obj_label['class_label'], 'class_confidence': obj_label['class_confidence'], 'frame_number': frame_number, 'count': 'return-in', 'timestamp': bbox['timestamp']})
                    elif prev_count == 0 and count == -1:
                        result.append({'object_id': obj_id, 'class_label' : obj_label['class_label'], 'class_confidence': obj_label['class_confidence'], 'frame_number': frame_number, 'count': 'out', 'timestamp': bbox['timestamp']})
                    elif prev_count == -1 and count == 0:
                        result.append({'object_id': obj_id, 'class_label' : obj_label['class_label'], 'class_confidence': obj_label['class_confidence'], 'frame_number': frame_number, 'count': 'return-out', 'timestamp': bbox['timestamp']})
        return result
    ...
    def add_bounding_box(self, object_id, bbox_xyxy, class_label, class_confidence):
        ...
        if self.is_webcam:
            ...
            timestamp = self.master.get_current_timestamp()
            tdelta = d.timedelta(seconds=timestamp)
            dt_start = d.datetime(1970, 1, 1)
            dt = dt_start + tdelta
            timestamp_txt = dt.strftime("%H:%M:%S")
            curr_frame_bbox_state = BBoxState(object_id, bbox_xyxy, class_label, class_confidence, timestamp=timestamp_txt)
